    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 bg-white/50 backdrop-blur-md px-2 py-1.5 rounded-full border border-gray-200/50 shadow-sm">
          {['الرئيسية', 'المميزات', 'لماذا نحن؟', 'الأسعار'].map((item, idx) => (
            <a 
              key={idx}
              href={`#${idx === 0 ? '' : 'features'}`} 
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`fixed inset-0 bg-white z-40 flex flex-col pt-28 px-6 transition-all duration-500 md:hidden ${
          isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'
        }`}
      >
        <div className="flex flex-col gap-2 text-lg font-medium text-center">
          {['الرئيسية', 'المميزات', 'لماذا نحن؟'].map((item, idx) => (
             <a key={idx} href="#" onClick={() => setIsMobileMenuOpen(false)} className="py-4 border-b border-gray-50 text-slate-800 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all">
               {item}
             </a>
          ))}
        </div>
